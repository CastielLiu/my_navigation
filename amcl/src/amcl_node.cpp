...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  //还未收到过此类型frame_id的雷达数据  //支持多雷达
  if(frame_to_laser_.find(laser_scan_frame_id) == frame_to_laser_.end())
  {
    ROS_DEBUG("Setting up laser %d (frame_id=%s)\n", (int)frame_to_laser_.size(), laser_scan_frame_id.c_str());
    lasers_.push_back(new AMCLLaser(*laser_));   //记录雷达模型
    ...
    //tf单位矩阵转换为geometry_msgs::Pose
    tf2::toMsg(tf2::Transform::getIdentity(), ident.pose);
  
    geometry_msgs::PoseStamped laser_pose;
    try
    {
      //雷达坐标下的单位变换(0变换)，转换到base_frame, 得到雷达在基坐标系下的位置laser_pose
      //src, dst, target_frame  其中src中包含src_frame,以及时间戳   
    ...
    laser_pose_v.v[2] = 0;
    lasers_[laser_index]->SetLaserPose(laser_pose_v); //将雷达位置写入模型
   ...
   //以上代码用于确定当前帧雷达数据属于哪一个雷达，若第一次收到，将初始化雷达相关信息。
  ...
  if(pf_init_)
  {
    ...
    //pf_odom_pose_ 为上一次amcl更新定位时的机器人姿态(相对于odom)
