...
double AMCLLaser::LikelihoodFieldModel(AMCLLaserData *data, pf_sample_set_t* set)
{
  ...
  for (j = 0; j < set->sample_count; j++)
  {
    ...
    double z_hit_denom = 2 * self->sigma_hit * self->sigma_hit; //sigma_hit 测量噪声的方差
    double z_rand_mult = 1.0/data->range_max; //随机噪声
    ...
    for (i = 0; i < data->range_count; i += step)
    {
      obs_range = data->ranges[i][0]; //观测距离
      obs_bearing = data->ranges[i][1]; //观测方位
      ...
      // 获取地图上离当前雷达点最近的障碍点距离
      // 其中有一个参数叫做max_occ_dist或者laser_likelihood_max_dist_，距离障碍物的最大距离
      // 当栅格距离大于最大距离时，设为最大距离
